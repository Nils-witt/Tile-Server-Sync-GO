...
// one shared replacer works for all pages. Status.html additionally
// contains real html/template actions (e.g. {{.StartedAt}}); those are
// untouched here since they don't match any of these four exact tokens, and
// are parsed as a template afterward by the caller (see webserver.go's
// pageTemplate).
